    ...
    <form action="{{ route('stok_barang.update', $stokBarang->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" id="tanggal" name="tanggal" 
                   value="{{ old('tanggal', $stokBarang->tanggal) }}" required>
        </div>

        <div class="mb-3">
            <label for="tipe" class="form-label">Tipe Transaksi</label>
            <select class="form-control" id="tipe" name="tipe" required>
                <option value="In" {{ old('tipe', $stokBarang->tipe) == 'In' ? 'selected' : '' }}>In</option>
                <option value="Out" {{ old('tipe', $stokBarang->tipe) == 'Out' ? 'selected' : '' }}>Out</option>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" id="quantity" name="quantity" 
                   value="{{ old('quantity', $stokBarang->quantity) }}" min="0" required>
        </div>
